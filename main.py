...
def crawl_group(group_id, group_name):
    print("=========================================================")
    print("开始爬取豆瓣小组：", group_name, "\n")
    print("开始执行，时间：", datetime.strftime(local_time(), "%Y-%m-%d %H:%M:%S") + "\n")

    ## 连接数据库、创建数据表
    rent_info = sqlite3.connect("rent_info.db")
    rent_info.execute(
        f"""CREATE TABLE IF NOT EXISTS {group_id}
                (title TEXT NOT NULL,
    ...
    cursor.execute(f"SELECT * FROM {group_id} ORDER BY update_time DESC")
    ...
    for page in range(1, max_page + 1):  # 从最新页开始爬取
        ...
        page_info = crawl_page(group_id, page)  # 爬取一页

        item_num_cur_page = 0  # 当前页新添加/更新的数据项
        for item in page_info:
            latest_update = max(latest_update, item[1])
            item_num_cur_page += add_item(cursor, group_id, item)
